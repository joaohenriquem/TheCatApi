...
export class ApiService {
  ...
  getCatsBreed(): Observable<any> {
    return this.httpClient.get<any>(enviremonent.BASE_URL + '/cats/listBreeds')
      .pipe(
        catchError((err) => {
          if (err.status === 0 && err.status !== 404) {
            this.utilsService.showError('Ocorreu um erro na aplicação, tente novamente!')
  ...
  getCatsBreedByOrigin(origin: string): Observable<any> {
    return this.httpClient.get<any>(enviremonent.BASE_URL + '/cats/listInfoByOrigin/'+ origin)
      .pipe(
        catchError((err) => {
          if (err.status === 0 && err.status !== 404) {
            this.utilsService.showError('Ocorreu um erro na aplicação, tente novamente!')
